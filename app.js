...
// Seal idle sheen (publish-safe)
(() => {
  const waxEl = document.getElementById('wax');
  if(!waxEl) return;
  let t=null;
  const arm=()=>{
    waxEl.classList.remove('idle');
    clearTimeout(t);
    t=setTimeout(()=>waxEl.classList.add('idle'), 4200);
  };
  ['pointerdown','touchstart','mousemove','keydown','scroll'].forEach(ev=>window.addEventListener(ev, arm, {passive:true}));
  arm();
})();
